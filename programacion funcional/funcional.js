...
//ejercicio 3
//Examine the code in the editor. It's using a method that has side effects in the program, causing incorrect behaviour. The final list of open tabs, stored in finalTabs.tabs, 
...
function incrementer() {
  // Only change code below this line
  let valueGreaterThan= fixedValue+1
...
ratings=watchList.map(elto=>(
  {
    title:elto.Title,
    rating:elto.imdbRating
  }
  ))
...
const filteredList= List.filter(elto=> elto.rating>=8)
...
function getRating(watchList) {
  // Only change code below this line
  const nolanData = watchList.reduce((data, { Director: director, imdbRating: rating }) => {
